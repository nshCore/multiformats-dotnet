...
namespace TheDotNetLeague.MultiFormats.MultiAddress
{
    /// <summary>
    ...
    public sealed class MultiAddress : IEquatable<MultiAddress>
    {
        ...
        ///     is "ipfs" or "p2p".  For example,
        ///     <c>/ip4/10.1.10.10/tcp/29087/ipfs/QmVcSqVEsvm5RR9mBLjwpb2XjFVn5bPdPL69mL8PH45pPC</c>.
        /// </remarks>
        public MultiHash.MultiHash PeerId
        {
            get
            {
        ...
        {
            get { return Protocols.Any(p => p.Name == "ipfs" || p.Name == "p2p"); }
        }
        ...
        private void Read(TextReader stream)
        {
            ...
            while (true)
            {
                ...
                if (!NetworkProtocol.Names.TryGetValue(name.ToString(), out var protocolType))
                    throw new FormatException(string.Format("The IPFS network protocol '{0}' is unknown.", name));
                var p = (NetworkProtocol) Activator.CreateInstance(protocolType);
                p.ReadValue(stream);
                Protocols.Add(p);
            }
